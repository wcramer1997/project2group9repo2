...
humans_wrangled <- humans %>%
  mutate(
    wrangled_subtypes = str_replace(subtypes, ", ", "")) %>%
  mutate(
    final_subtypes =  str_replace(wrangled_subtypes, ", ", ""))

# grepl should work better now

humans_wrangled %>%
  filter(grepl("Wizard", final_subtypes)) # returns 782 rows 

 # not sure how to make this function work, but if I can, we can use it to find subtypes within subtypes really easily with lapply  

find_jobs_function <- 
  function(job_type) 
  {creatures_wrangled %>%
      filter(
      grepl("job_type", final_subtypes))
  }
...
filter(
  subtypes == "Zombie" | subtypes == "Goblin" | subtypes == "Treefolk" | subtypes == "Dragon") %>%
group_by(
